...
const LOCAL_FUND_50_FIELDS = [
  { key: "ssOffering", label: "SS Offering" },
...
export function LocalFund50ReportPage() {
  const { state } = useData();
  const years = getAvailableYears(state.localFundEntries);
  ...
  const rows = useMemo(() => filterEntries(state.localFundEntries, filters, state.settings.quarters), [state.localFundEntries, filters, state.settings.quarters]);
  ...
  return (
    <div className="fund-report-screen local-fund-book-screen">
      <PageHeader title="Local Fund (50%) Report" subtitle="Treasurer's report book for 50% local fund entries." />
      ...
      <article className="local-fund-book-page" id="local-fund-50-report-print">
          ...
          <table className="local-fund-book-table">
            <thead>
              <tr>
                ...
                <th>Received From</th>
                <th>Receipt<br />No.</th>
                {LOCAL_FUND_50_FIELDS.map((field) => <th key={field.key}>{field.label}</th>)}
                <th>Total</th>
              </tr>
            </thead>
            <tbody>
              {rows.map((entry) => (
                <tr key={entry.id}>
                  ...
                  {LOCAL_FUND_50_FIELDS.map((field) => <td className="amount-cell" key={field.key}>{moneyOrBlank(entry.amounts?.[field.key], state.settings.currencySymbol)}</td>)}
                  <td className="amount-cell">{money(entry.totalAmount, state.settings.currencySymbol)}</td>
                </tr>
              ))}
              {!rows.length && (
                <tr>
                  <td className="empty-book-row" colSpan={LOCAL_FUND_50_FIELDS.length + 4}>No records found.</td>
                </tr>
              )}
              <tr className="book-total-row">
                <td colSpan={LOCAL_FUND_50_FIELDS.length + 2} />
                <th>Grand Total</th>
                <td className="amount-cell">{money(totals.grandTotal, state.settings.currencySymbol)}</td>
              </tr>
              <tr className="book-half-row">
                <td colSpan={LOCAL_FUND_50_FIELDS.length + 2} />
                <th>50%</th>
                <td className="amount-cell">{money(totals.half, state.settings.currencySymbol)}</td>
              </tr>
            </tbody>
          </table>
        </div>
...
function calculateTotals(rows) {
  const grandTotal = rows.reduce((sum, entry) => sum + toNumber(entry.totalAmount), 0);
  return { grandTotal, half: grandTotal * 0.5 };
}
